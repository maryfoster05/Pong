class Ball{
    ...
    hitPaddleLeft(paddle1){
        ...
        this.x - this.r < paddle1.x + paddle1.w / 2 ) 
        {
            if (this.x > paddle1.x){
                ...
                let angle = map (diff, 0, paddle1.h, -radians(45), radians(45));
                this.xSpeed = 5 * Math.cos(angle);
                this.ySpeed = 5 * Math.sin(angle);
                this.x = paddle1 + paddle1.w / 2 + this.r;
    ...
    hitPaddleRight(paddle2){
        ...
        this.x + this.r > paddle2.x - paddle2.w /2 ) 
        {
            if (this.x < paddle2.x){
                ...
                this.xSpeed = 5 * Math.cos(angle);
                this.ySpeed = 5 * Math.sin(angle);
                this.x = paddle2 - paddle2.w / 2 - this.r;
                this.updateBallPosition();
            }
        }
    }
    ...
    checkEdges(){
        ...
        if (this.x - this.r > width){
            this.backToOrgin();
            // this.sound.play(); 
            // eventually this will change the left score
        }

        if (this.x + this.r < 0){
            this.backToOrgin();
            // this.Sound.play();
            // eventually this will change the right score
        }
    }
